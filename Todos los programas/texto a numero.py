text = input("Ingresa número de tres cifras en texto: \n").lower().replace("á", "a").replace("é", "e").replace("í", "i").replace("ó", "o").replace("ú", "u")
text = text.split()

unidad = text[-1]
num = 0
if "y" in text: del text[-2]
if "dieci" in text[-1]: 
    text[-1] = unidad[:5]
    text.append(unidad[5:])
elif "veinti" in text[-1]: 
    text[-1] = unidad[:6]
    text.append(unidad[6:])
    
unidad = text[-1]
decena, centena = "", ""
if len(text) > 1: decena = text[-2]
...
elif (decena != "" or centena != "") and (decena != "dieci" and decena != "veinti"): num = 0

if (unidad in c):
    if c.index(unidad) != 0: num += 100*(c.index(unidad)+1)
elif unidad == "cien": num =100

if num == 0 and unidad != "cero": print(f"\n¡¡Error!! \nSe desconoce al número '{" ".join(text)}'\nPorfavor, verifique que el número este escrito correctamente e intente de nuevo \n")
else: print(f"\nTu número es: {num}\n")
